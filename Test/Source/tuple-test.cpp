...
{
	count = 0;
	Utility::call_with_tuple_elements(PassTupleElementsTestFunction1, std::tuple<IntegerType<0>>());
	EXPECT_EQ(count, 1);
	count = 0;
	Utility::call_with_tuple_elements(PassTupleElementsTestFunction2, std::tuple<IntegerType<0>, IntegerType<1>>());
	EXPECT_EQ(count, 2);
	count = 0;
	Utility::call_with_tuple_elements(PassTupleElementsTestFunction3, std::tuple<IntegerType<0>, IntegerType<1>, IntegerType<2>>());
	EXPECT_EQ(count, 3);
	count = 0;
	Utility::call_with_tuple_elements(PassTupleElementsTestFunction4, std::tuple<IntegerType<0>, IntegerType<1>, IntegerType<2>, IntegerType<3>>());
	EXPECT_EQ(count, 4);
	count = 0;
